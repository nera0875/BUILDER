...
import { getTasks, reorderTasks, type TaskReorderUpdate } from "@/app/actions/task-actions";
    ...
    async function fetchData() {
      ...
      if (result.success && result.data) {
        ...
        Object.entries(result.data).forEach(([status, tasks]) => {
          columnsData[status as TaskStatus] = tasks as Task[];
        });
              ...
              <Kanban.Column
                ...
                {tasks.length > 0 ? (
                  ...
                  <div className="flex flex-col justify-center gap-4 pt-4">
                    <div className="text-muted-foreground text-sm">No tasks in {statusConfig.title.toLowerCase()}.</div>
                  </div>
                )}
              </Kanban.Column>
            );
